...
?>
		...
		?>
			...
			?>
				<div class="abs-list-sec right_pos <?= (($image[0]['company_web_title'] != '') ? "" : "d-ddnone"); ?>">

					<a href="<?= base_url() ?>sahar/<?= url_title($image[0]['company_city']) ?>/<?= url_title(strtolower($cate[0]['category'])); ?>/<?= $image[0]['company_web_title'] ?>" class="add-list-btn" target="_blank"><i class="fas fa-id-card me-2"></i>Visit your vcard/website</a>

				</div>


				<div class="abs-list-sec <?= (($image[0]['company_web_title'] != '') ? "" : "d-dd"); ?>">
					<a href="<?= base_url('product-add') ?>" class="add-list-btn"><i class="fas fa-plus me-2"></i>Add Product</a>
				</div>
						...
						<div class="modal-body p-5">
							...
							<form class="submit-form" method="POST" enctype="multipart/form-data" action="<?= base_url('Home/banner_update') ?>">
								<div class="form-group">
									...
									<input type="file" class="form-control rounded" name="company_card_background_temp" accept=".png,.jpg,.jpeg" />
